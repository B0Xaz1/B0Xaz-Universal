...
return function(Context)
    ...
    function AimbotSystem.UpdateAim()
        ...
        _lastAimScreenPos = _lastAimScreenPos:Lerp(sp, 0.15)
        ...
        local ok = pcall(function() mousemoverel(step.X, step.Y) end)
        if not ok then 
    ...
    function AimbotSystem.UpdateTriggerbot()
        ...
        for _, p in ipairs(Players:GetPlayers()) do
            ...
            for _, part in ipairs(char:GetChildren()) do
                ...
                if (mousePos - sp).Magnitude < 12 then
                    task.delay(FeatureConfig.Aimbot.Triggerbot.Delay, function()
                        if not FeatureConfig.Aimbot.Triggerbot.Enabled then return end
                        local vui = game:GetService("VirtualInputManager")
                        pcall(function() vui:SendMouseButtonEvent(0, 0, 0, true, game, 1) end)
                        task.wait(0.05)
                        pcall(function() vui:SendMouseButtonEvent(0, 0, 0, false, game, 1) end)
                    end)
                    return
                end
            end
