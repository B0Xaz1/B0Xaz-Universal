...
return function(CONFIG)
    ...
    function Utils.GetMousePosition() 
        if IsMobile then return Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2) end
        return UIS:GetMouseLocation() 
    end
    function Utils.GetScreenCenter() 
        return Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2) 
    end
    function Utils.ColorToTable(c) return {r = c.R, g = c.G, b = c.B} end
    function Utils.TableToColor(t) 
        if not t then return Color3.new(1,1,1) end
        return Color3.new(math.clamp(tonumber(t.r) or 1, 0, 1), math.clamp(tonumber(t.g) or 1, 0, 1), math.clamp(tonumber(t.b) or 1, 0, 1)) 
    end
    ...
    function Utils.SanitizeFileName(name) 
        ...
        if #out > CONFIG.CONFIG_NAME_MAX_LEN then out = out:sub(1, CONFIG.CONFIG_NAME_MAX_LEN) end
        return out 
    end
    function Utils.SafeCall(fn, ...) 
        if type(fn) ~= "function" then return end
        local ok, err = pcall(fn, ...)
        if not ok then pcall(function() warn("[B0Xaz] " .. tostring(err)) end) end 
    end
