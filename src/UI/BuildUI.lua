...
return function(Context)
	...
	-- TIER 3: Aimbot
	----------------------------------------------------------------
	local aimbotTab = UI:AddTab("Aimbot", 3)
	...
	UIRegistry.Aimbot_VisCheck = aimCheck:AddToggle("Visibility Check (Raycast)", FeatureConfig.Aimbot.VisCheck, function(v) FeatureConfig.Aimbot.VisCheck = v end)
	UIRegistry.Aimbot_UnlockOnDeath = aimCheck:AddToggle("Unlock On Target Death", FeatureConfig.Aimbot.UnlockOnDeath, function(v) FeatureConfig.Aimbot.UnlockOnDeath = v end)
	UIRegistry.Aimbot_BreakOnPull = aimCheck:AddToggle("Break Lock On Mouse Pull", FeatureConfig.Aimbot.BreakOnPull, function(v) FeatureConfig.Aimbot.BreakOnPull = v end)
	...
	UIRegistry.Aimbot_Prediction_Horizontal = aimPred:AddSlider("Horizontal Lead", math.floor((FeatureConfig.Aimbot.Prediction.Horizontal or 0) * 200), 0, 100, function(v) FeatureConfig.Aimbot.Prediction.Horizontal = v / 200 end)
	UIRegistry.Aimbot_Prediction_Vertical = aimPred:AddSlider("Vertical Lead", math.floor((FeatureConfig.Aimbot.Prediction.Vertical or 0) * 200), 0, 100, function(v) FeatureConfig.Aimbot.Prediction.Vertical = v / 200 end)
	...
	UIRegistry.Aimbot_Triggerbot_Enabled = aimTrig:AddToggle("Enable Triggerbot", FeatureConfig.Aimbot.Triggerbot.Enabled, function(v) FeatureConfig.Aimbot.Triggerbot.Enabled = v end)
	UIRegistry.Aimbot_Triggerbot_Delay = aimTrig:AddSlider("Trigger Delay", math.floor(FeatureConfig.Aimbot.Triggerbot.Delay * 100), 0, 50, function(v) FeatureConfig.Aimbot.Triggerbot.Delay = v / 100 end, " ms")

	----------------------------------------------------------------
	-- TIER 2: ESP
	----------------------------------------------------------------
	local espTab = UI:AddTab("ESP", 2)
	...
	----------------------------------------------------------------
	-- TIER 1: Players (Fling restricted to Tier 3)
	----------------------------------------------------------------
	...
	plSelectSec:AddToggle("Include LocalPlayer (Testing)", includeSelf, function(v) includeSelf = v; refreshList() end)
	...
	plActionsSec:AddButton("Teleport to Player", function()
		if not selectedName or selectedName == "No Players Found" then
			UI:Notify("Players", "Select a valid player first", nil, Theme.Danger); return
		end
	...
	plStatusSec:AddButton("Spectating: None", function() PlayersSystem.StopSpectate() end)
	plStatusSec:AddButton("Flinging: None", function() PlayersSystem.StopFling() end, 3)
			...
			if plStatusSec and plStatusSec.Frame then
				for _, elem in ipairs(plStatusSec.Frame:GetDescendants()) do
					if elem:IsA("TextButton") then
						if elem.Text:find("Spectating:") then elem.Text = specText
						elseif elem.Text:find("Flinging:") then elem.Text = flingText end
					end
				end
			end
			task.wait(0.5)
	...
	UIRegistry.Movement_SprintSpeed = charSec:AddSlider("Sprint Speed", FeatureConfig.Movement.SprintSpeed or 30, 16, 500, function(v) FeatureConfig.Movement.SprintSpeed = v end, " ws")
	...
	UIRegistry.Movement_CFrameSpeed = charSec:AddToggle("CFrame Movement", FeatureConfig.Movement.CFrameSpeed, function(v) FeatureConfig.Movement.CFrameSpeed = v end)
	UIRegistry.Movement_CFrameSpeedValue = charSec:AddSlider("CFrame Move Speed", FeatureConfig.Movement.CFrameSpeedValue or 50, 1, 500, function(v) FeatureConfig.Movement.CFrameSpeedValue = v end, " sps")
	UIRegistry.Movement_Bhop = charSec:AddToggle("Bhop (Hold Space)", FeatureConfig.Movement.Bhop, function(v) FeatureConfig.Movement.Bhop = v end)
	...
	worldSec:AddSlider("Camera Zoom", 400, 10, 1000, function(v) LocalPlayer.CameraMaxZoomDistance = v end)

	local tpSec = moveTab:AddSection("Teleports")
	tpSec:AddButton("Save Position", function()
		local r = Utils.GetRootPart()
		if not r then UI:Notify("Position", "No character root", nil, Theme.Danger); return end
		State.SavedPosition = r.CFrame
		UI:Notify("Position", "Saved", nil, Theme.Success)
	end)
	tpSec:AddButton("Return to Saved", function()
		local r = Utils.GetRootPart()
		if r and State.SavedPosition then r.CFrame = State.SavedPosition
			UI:Notify("Position", "Teleported", nil, Theme.Success)
		else UI:Notify("Position", "No saved position", nil, Theme.Danger) end
	end)
	...
	-- TIER 1: Optimization
	----------------------------------------------------------------
	local optTab = UI:AddTab("Optimization", 1)
	...
	UIRegistry.Perf_LowMaterials = gpuSec:AddToggle("Force Smooth Plastic", FeatureConfig.Performance.LowMaterials, function(v) PerfSystem.SetLowMaterials(v) end)
	UIRegistry.Perf_OptimizeTerrain = gpuSec:AddToggle("Optimize 3D Terrain", FeatureConfig.Performance.OptimizeTerrain, function(v) PerfSystem.SetOptimizeTerrain(v) end)
	UIRegistry.Perf_NoPostProcessing = gpuSec:AddToggle("Disable Post-Processing", FeatureConfig.Performance.NoPostProcessing, function(v) PerfSystem.SetNoPostProcessing(v) end)

	local fxSec = optTab:AddSection("Effects Optimizer")
	UIRegistry.Perf_NoShadows = fxSec:AddToggle("Disable Part Shadows", FeatureConfig.Performance.NoShadows, function(v) PerfSystem.SetNoShadows(v) end)
	UIRegistry.Perf_NoParticles = fxSec:AddToggle("Disable Particles", FeatureConfig.Performance.NoParticles, function(v) PerfSystem.SetNoParticles(v) end)

	local fpsSec = optTab:AddSection("Framerate")
	fpsSec:AddButton("Unlock FPS (999)", function() pcall(function() setfpscap(999) end) UI:Notify("Opt", "FPS Unlocked", nil, Theme.Success) end)
	fpsSec:AddButton("Reset 60 FPS", function() pcall(function() setfpscap(60) end) UI:Notify("Opt", "60 FPS", nil, Theme.Warning) end)
	fpsSec:AddButton("Cap 144 FPS", function() pcall(function() setfpscap(144) end) UI:Notify("Opt", "144 FPS", nil, Theme.Success) end)
	...
	infoSec:AddButton("PlaceId: " .. tostring(game.PlaceId), function()
		...
		UI:Notify("Game", "PlaceId copied", nil, Theme.Accent)
	end)
	...
	-- TIER 2: Extras
	----------------------------------------------------------------
	local extrasTab = UI:AddTab("Extras", 2)
	...
	miscSec:AddButton("Server Hop", function()
		...
		task.spawn(function()
			...
			if not ok or not servers or not servers.data then
				UI:Notify("Server Hop", "Failed to fetch servers", nil, Theme.Danger); return
			end
	...
	-- TIER 1: Themes
	----------------------------------------------------------------
	local themeTab = UI:AddTab("Themes", 1)
	...
	if ThemeManager and ThemeManager.Presets then
		for pName, _ in pairs(ThemeManager.Presets) do table.insert(presetNames, pName) end
	...
	local function updateCustomPickers(newColors)
		local function syncCP(key, val)
			if UIRegistry[key] and UIRegistry[key].Set then UIRegistry[key].Set(val, true) end
		end
	...
	themePresetsSec:AddDropdown("Choose Preset", presetNames, function(v)
		if ThemeManager and ThemeManager.Presets and ThemeManager.Presets[v] then
			...
			UI:Notify("Themes", "Applied " .. v, nil, Theme.Success)
		end
	end, ThemeManager and ThemeManager.ActivePreset or "Default Cyan")

	UIRegistry.Theme_Accent = themeCustomSec:AddColorPicker("Accent Color", Theme.Accent, function(c) UI:UpdateThemeKey("Accent", c) end)
	UIRegistry.Theme_Bg = themeCustomSec:AddColorPicker("Main Background", Theme.Bg, function(c) UI:UpdateThemeKey("Bg", c) end)
	UIRegistry.Theme_Panel = themeCustomSec:AddColorPicker("Panel Background", Theme.Panel, function(c) UI:UpdateThemeKey("Panel", c) end)
	UIRegistry.Theme_Elem = themeCustomSec:AddColorPicker("Element / Button", Theme.Elem, function(c) UI:UpdateThemeKey("Elem", c) end)
	UIRegistry.Theme_Side = themeCustomSec:AddColorPicker("Header / Sidebar", Theme.Side, function(c) UI:UpdateThemeKey("Side", c) end)
	UIRegistry.Theme_Text = themeCustomSec:AddColorPicker("Text Primary", Theme.Text, function(c) UI:UpdateThemeKey("Text", c) end)
	UIRegistry.Theme_Border = themeCustomSec:AddColorPicker("Border Outline", Theme.Border, function(c) UI:UpdateThemeKey("Border", c) end)
	UIRegistry.Theme_ToggleOn = themeCustomSec:AddColorPicker("Toggle Active", Theme.ToggleOn, function(c) UI:UpdateThemeKey("ToggleOn", c) end)

	----------------------------------------------------------------
	-- TIER 1: Config Tab (Key Manager + Configs)
	----------------------------------------------------------------
	local cfgTab = UI:AddTab("Config", 1)
	...
	keySec:AddButton("Copy Key Info", function()
	...
	-- ======== Config Manager ========
	local cfgSec = cfgTab:AddSection("Manage Configs")
	...
	cfgSec:AddButton("Refresh Config List", function()
		...
		UI:Notify("Configs", "Refreshed", nil, Theme.Accent)
	end)
	...
	cfgSec:AddButton("Save Config", function()
		...
		if name:lower() == "default" then UI:Notify("Config", "Cannot overwrite Default", nil, Theme.Warning); return end
		local ok, err = ConfigSystem.Save(name)
		if ok then cfgDropdown.Refresh(getCfgList(), true); UI:Notify("Saved", name, nil, Theme.Success)
		else UI:Notify("Failed", tostring(err), nil, Theme.Danger) end
	end)

	cfgSec:AddButton("Load Selected", function()
		if not cfgState.Selected then UI:Notify("Config", "Select first", nil, Theme.Danger); return end
		local ok, err = ConfigSystem.Load(cfgState.Selected)
		if ok then UI:Notify("Loaded", cfgState.Selected, nil, Theme.Success)
		else UI:Notify("Failed", tostring(err), nil, Theme.Danger) end
	end)

	cfgSec:AddButton("Delete Selected", function()
		if not cfgState.Selected then UI:Notify("Config", "Select first", nil, Theme.Danger); return end
		if cfgState.Selected == "Default" then UI:Notify("Config", "Default cannot be deleted", nil, Theme.Warning); return end
	...
	cfgIO:AddButton("Copy Config to Clipboard", function()
		...
		if ok then
			...
			if ok2 then UI:Notify("Export", "Copied", nil, Theme.Success)
			else UI:Notify("Export", "Clipboard unavailable", nil, Theme.Danger) end
		else UI:Notify("Export", "Encode failed", nil, Theme.Danger) end
	end)
	...
	cfgIO:AddButton("Import Config from Textbox", function()
		if not rawImportString or #rawImportString == 0 then
			UI:Notify("Import", "Paste config first", nil, Theme.Danger); return
		end
		...
		if ok and type(data) == "table" then
			...
			if saved then
				...
				UI:Notify("Import", "Applied only: " .. tostring(saveErr), nil, Theme.Warning)
			end
		else
	...
	-- Menu toggle key row (custom row inside Config)
	do
		...
		table.insert(cfgSec.Elements, {Container = row, Name = "Menu Toggle Key"})
