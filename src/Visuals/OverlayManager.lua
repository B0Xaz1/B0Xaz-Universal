...
return function(Context)
    ...
    if DrawingManager.Available then
        for i = 1, CONFIG.SPEED_LINES_COUNT do
            ...
            if l then 
                ...
                speedLineData[i] = {
                    ...
                    dist = math.random(CONFIG.SPEED_LINES_MIN_DIST, CONFIG.SPEED_LINES_MAX_DIST)
                } 
            end
        end
    end
    ...
    function OverlayManager.UpdateSpeedLines(dt)
        ...
        for i, line in ipairs(speedLines) do
            ...
            d.dist += dt * CONFIG.SPEED_LINES_SPEED
            if d.dist > CONFIG.SPEED_LINES_MAX_RANGE then 
                d.dist = math.random(CONFIG.SPEED_LINES_SPAWN_MIN, CONFIG.SPEED_LINES_SPAWN_MAX)
                d.angle = math.random() * math.pi * 2 
            end
            local cos, sin = math.cos(d.angle), math.sin(d.angle)
            line.From = Vector2.new(c.X + cos * d.dist, c.Y + sin * d.dist)
            line.To = Vector2.new(c.X + cos * (d.dist + CONFIG.SPEED_LINES_LENGTH), c.Y + sin * (d.dist + CONFIG.SPEED_LINES_LENGTH))
            line.Transparency = math.clamp(1 - (d.dist / CONFIG.SPEED_LINES_MAX_RANGE), 0.1, 1)
            line.Visible = true
        end
    end
